...
RUN apt-get update && apt-get install unzip pasmo && chmod 700 *.sh && unzip jsspeccy-3.1.zip && mv jsspeccy-3.1/jsspeccy /var/www/html && rm -rf jsspeccy-3.1 jsspeccy-3.1.zip && apt-get remove wget unzip -y
